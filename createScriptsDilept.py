...
for chan in channels:
    ...
    for d in datasets:
        if d.attrib['add'] == '1':
            ...
            shutil.copyfile("submitTestSkeletonDilept.sh", filenameTest)
            # append to the file the actual command 
            outfileTest = open (filenameTest, 'a')
            if not len(topTrees) == 0:
                print >> outfileTest, commandString, topTrees[0]
...
#                    print "len(listOfFiles) is ", len(listOfFiles) 
                    ...
                    shutil.copyfile("submitSkeletonDilept.sh", filename)
...
#                    print files_str
                    ...
                    if "Dilep" in chan:
                        commandString = "./MACRO "+str(d.attrib['name'])+"_"+str(N_job)+" "+str(d.attrib['title'])+" "+str(d.attrib['add'])+" "+str(d.attrib['color'])+" "+str(d.attrib['ls'])+" "+str(d.attrib['lw'])+" "+str(d.attrib['normf'])+" "+str(d.attrib['EqLumi'])+" "+str(d.attrib['xsection'])+" "+str(d.attrib['PreselEff'])


                    # run on the files
                    print >> outfile, "# now run on the file copied under /$TMPDIR/ "
                    print >> outfile, commandString, scractFiles_str
