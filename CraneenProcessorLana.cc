...
void SplitDatasetPlotter(int nBins, float lScale, float plotLow, float plotHigh, string leptoAbbr, TFile *shapefile, TFile *errorfile, string channel, string sVarofinterest, string sSplitVar, float fbSplit, float ftSplit, float fwSplit, string xmlNom, string CraneenPath, string shapefileName);
...
void GetScaleEnvelopeSplit(int nBins, float lScale, float plotLow, float plotHigh, string leptoAbbr, TFile *shapefile, TFile *errorfile, string channel, string sVarofinterest, string sSplitVar, float fbSplit, float ftSplit, float fwSplit, string xmlNom, string CraneenPath, string shapefileName, string mainTTbarSample);
...

    {
        ...
        for (child2; child2; child2=child2->NextSiblingElement()){
            ...
            if (ppKey && ppText) 
            {
                ...
                // shapefileName = "shapefile"+leptoAbbr+"_"+"DATA"+"_"+VoI+"_"+splitting+"_L30_pt31.root";
                ...
                if(jetSplit)
                {
                    SplitDatasetPlotter(NumberOfBins, lumiScale, lBound, uBound, leptoAbbr, shapefile, errorfile, channel, VoI, splitVar, bSplit, tSplit, wSplit, xmlFileName, CraneenPath, shapefileName);
...
void GetScaleEnvelope(int nBins, float lScale, float plotLow, float plotHigh, string leptoAbbr, TFile *shapefile, TFile *errorfile, string channel, string sVarofinterest, string xmlNom, string CraneenPath, string shapefileName, string mainTTbarSample){
    ...
    histo1D["weightPlus_all"] = new TH1F("Plus_all","Plus_all", nBins, plotLow, plotHigh);
    histo1D["weightMinus_all"] = new TH1F("Minus_all","Minus_all", nBins, plotLow, plotHigh);
...
void GetMatching(int nBins, float lScale, float plotLow, float plotHigh, string leptoAbbr, TFile *shapefile, TFile *errorfile, string channel, string sVarofinterest, string xmlNom, string CraneenPath, string mainTTbarSample, string otherTTbarsample){
    ...
    histo1D["other_ttbar"] = (TH1F*)shapefile->Get((channel+"__"+otherTTbarsample+"__nominal").c_str());    cout<<"Got matching histos"<<endl;
...
void GetMatchingSplit(int nBins, float lScale, float plotLow, float plotHigh, string leptoAbbr, TFile *shapefile, TFile *errorfile, string channel, string sVarofinterest, string xmlNom, string CraneenPath, string mainTTbarSample, string otherTTbarsample,string sSplitVar, float fbSplit, float ftSplit, float fwSplit){
    ...
    {    
        ...
        histo1D["other_ttbar"] = (TH1F*)shapefile->Get((channel+numStr+sSplitVar+"__"+otherTTbarsample+"__nominal").c_str());    cout<<"Got matching histos"<<endl;
...
void GetScaleEnvelopeSplit(int nBins, float lScale, float plotLow, float plotHigh, string leptoAbbr, TFile *shapefile, TFile *errorfile, string channel, string sVarofinterest, string sSplitVar, float fbSplit, float ftSplit, float fwSplit, string xmlNom, string CraneenPath, string shapefileName, string mainTTbarSample){
    ...
    for(int s = fbSplit; s <= ftSplit; s+=fwSplit)
    {
        ...
        histo1D[("weight0"+numStr).c_str()] = (TH1F*)shapefile->Get(("Genweight_tt"+numStr).c_str());
        histo1D[("weight1"+numStr).c_str()] = (TH1F*)shapefile->Get(("weight1"+numStr).c_str());
        histo1D[("weight2"+numStr).c_str()] = (TH1F*)shapefile->Get(("weight2"+numStr).c_str());
        histo1D[("weight3"+numStr).c_str()] = (TH1F*)shapefile->Get(("weight3"+numStr).c_str());
        histo1D[("weight4"+numStr).c_str()] = (TH1F*)shapefile->Get(("weight4"+numStr).c_str());
        histo1D[("weight5"+numStr).c_str()] = (TH1F*)shapefile->Get(("weight5"+numStr).c_str());
        histo1D[("weight6"+numStr).c_str()] = (TH1F*)shapefile->Get(("weight6"+numStr).c_str());
        histo1D[("weight7"+numStr).c_str()] = (TH1F*)shapefile->Get(("weight7"+numStr).c_str());
        histo1D[("weight8"+numStr).c_str()] = (TH1F*)shapefile->Get(("weight8"+numStr).c_str());
        ...
        histo1D[("weight0"+numStr).c_str()]->Write("Nominal");
        cout<<"wrote weights in errorfile"<<endl;
        shapefile->cd();
...
void DatasetPlotter(int nBins, float lScale, float plotLow, float plotHigh, string leptoAbbr, TFile *shapefile, TFile *errorfile, string channel, string sVarofinterest, string xmlNom, string CraneenPath, string shapefileName)
{
    ...
    int nEntries;
    float ScaleFactor=1, NormFactor=1, Luminosity, varofInterest, GenWeight=1, weight1=1, weight2=1, weight3=1, weight4=1, weight5=1, weight6=1, weight7=1, weight8=1, ttbar_flav=1, SFlepton=1, SFbtag=1, SFbtagUp=1, SFbtagDown=1, SFPU=1, SFPU_up=1, SFPU_down=1, sfTopPt=1;
    ...
    histo1D["allBackgrounds"] = new TH1F("allBackgrounds","allBackgrounds", nBins, plotLow, plotHigh);
    for (int d = 0; d < datasets.size(); d++)  //Loop through datasets
    {
        ...
        // if(dataSetName.find("Data")==string::npos && dataSetName.find("JES")==string::npos && dataSetName.find("JER")==string::npos && dataSetName.find("Scale")==string::npos){
        //     histo1D["Genweight_all"] = new TH1F("Genweight","Genweight", nBins, plotLow, plotHigh);
        //     histo1D["weight1_all"] = new TH1F("weight1","weight1", nBins, plotLow, plotHigh);
        //     histo1D["weight2_all"] = new TH1F("weight2","weight2", nBins, plotLow, plotHigh);
        //     histo1D["weight3_all"] = new TH1F("weight3","weight3", nBins, plotLow, plotHigh);
        //     histo1D["weight4_all"] = new TH1F("weight4","weight4", nBins, plotLow, plotHigh);
        //     histo1D["weight5_all"] = new TH1F("weight5","weight5", nBins, plotLow, plotHigh);
        //     histo1D["weight6_all"] = new TH1F("weight6","weight6", nBins, plotLow, plotHigh);
        //     histo1D["weight7_all"] = new TH1F("weight7","weight7", nBins, plotLow, plotHigh);
        //     histo1D["weight8_all"] = new TH1F("weight8","weight8", nBins, plotLow, plotHigh);             
        // }
   
        if(dataSetName.find(mainTTbarSample)!=string::npos && dataSetName.find("JES")==string::npos && dataSetName.find("JER")==string::npos && dataSetName.find("Scale")==string::npos){
        ...
        float ProjectedLumi = 30000; //pb-1
        float LumiFactor = 1;
        ...
        for (int j = 0; j<nEntries; j++)
        {
            ...
            else
            {
                ...
                NormFactor =1;
                if (dataSetName.find("tttt")!=string::npos) {    NormFactor = 0.8452;    }
                else if (dataSetName.find("WJets")!=string::npos){     NormFactor = 0.9485;    }
                ...
                histo1D[dataSetName]->Fill(varofInterest,NormFactor*ScaleFactor*GenWeight*Luminosity*eqlumi*ttbbReweight*sfTopPt);
                if(dataSetName.find("Data")==string::npos && dataSetName.find("JES")==string::npos &&dataSetName.find("JER")==string::npos && dataSetName.find("Scale")==string::npos){
                    histo1D["allBackgrounds"]->Fill(varofInterest,NormFactor*ScaleFactor*GenWeight*Luminosity*eqlumi*ttbbReweight*sfTopPt);
                    // histo1D["Genweight_all"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*GenWeight*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    // histo1D["weight1_all"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight1*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    // histo1D["weight2_all"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight2*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    // histo1D["weight3_all"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight3*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    // histo1D["weight4_all"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight4*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    // histo1D["weight5_all"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight5*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    // histo1D["weight6_all"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight6*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    // histo1D["weight7_all"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight7*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    // histo1D["weight8_all"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight8*eqlumi*ttbbReweight*LumiFactor*sfTopPt);  
                }
                // SFbtag = 1;
                // ScaleFactor=1;
                // SFPU=1;
                ...
                // ttbbReweight=1;
                // cout<<SFbtag<< "   "<<  ScaleFactor<<  "   "<< SFPU<<  "   "<< SFlepton<< "   "<<  ttbbReweight<<    endl;
                // cout<<dataSetName<<"  "<<sfTopPt<<endl;
                if (dataSetName.find(mainTTbarSample)!=string::npos && dataSetName.find("JES")==string::npos &&dataSetName.find("JER")==string::npos && dataSetName.find("Scale")==string::npos){

                    histo1D["Genweight_tt"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*GenWeight*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    histo1D["weight1_tt"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight1*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    histo1D["weight2_tt"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight2*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    histo1D["weight3_tt"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight3*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    histo1D["weight4_tt"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight4*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    histo1D["weight5_tt"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight5*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    histo1D["weight6_tt"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight6*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    histo1D["weight7_tt"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight7*eqlumi*ttbbReweight*LumiFactor*sfTopPt);
                    histo1D["weight8_tt"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*weight8*eqlumi*ttbbReweight*LumiFactor*sfTopPt);   

                    histo1D["PU_Up"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU_up*Luminosity*GenWeight*eqlumi*ttbbReweight*LumiFactor*sfTopPt);   
                    histo1D["PU_Down"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU_down*Luminosity*GenWeight*eqlumi*ttbbReweight*LumiFactor*sfTopPt);   
                    histo1D["btag_Up"]->Fill(varofInterest,NormFactor*SFlepton*SFbtagUp*SFPU*Luminosity*GenWeight*eqlumi*ttbbReweight*LumiFactor*sfTopPt);   
                    histo1D["btag_Down"]->Fill(varofInterest,NormFactor*SFlepton*SFbtagDown*SFPU*Luminosity*GenWeight*eqlumi*ttbbReweight*LumiFactor*sfTopPt);   
                    histo1D["heavyFlav_Up"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*GenWeight*eqlumi*ttbbReweight_up*LumiFactor*sfTopPt);   
                    histo1D["heavyFlav_Down"]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*GenWeight*eqlumi*ttbbReweight_down*LumiFactor*sfTopPt);            
                     // cout<<"njets: "<<nJets<<endl;
                    if(split_ttbar){
                        // if (ttbar_flav>1.5) cout<<"ttbar_flav:  "<<ttbar_flav<<endl;
                        if(ttbar_flav<1.5){//light and charm
                            
                            MSPlot[plotname]->Fill(varofInterest, ttbar_ll, true, NormFactor*GenWeight*SFlepton*SFbtag*SFPU*Luminosity*ttbbReweight*LumiFactor*sfTopPt);                
                        }     
                        else{
                            MSPlot[plotname]->Fill(varofInterest, ttbar_bb, true, NormFactor*GenWeight*SFlepton*SFbtag*SFPU*Luminosity*ttbbReweight*LumiFactor*sfTopPt);                
                        }          
                    }
                    else{
                        MSPlot[plotname]->Fill(varofInterest, datasets[d], true, NormFactor*GenWeight*SFlepton*SFbtag*SFPU*Luminosity*ttbbReweight*LumiFactor*sfTopPt);   
                    }             
                }
                else if (dataSetName.find("tttt")!=string::npos){
                    MSPlot[plotname]->Fill(varofInterest, datasets[d], true, NormFactor*GenWeight*SFlepton*SFbtag*SFPU*Luminosity*ttbbReweight*LumiFactor*sfTopPt);
                }
                else if(dataSetName.find("MLM")==string::npos && dataSetName.find("Scale")==string::npos && dataSetName.find("JES")==string::npos && dataSetName.find("JER")==string::npos){ //ie. don't add the MLM dataset which is just used for matching
                    MSPlot[plotname]->Fill(varofInterest, datasets[d], true, NormFactor*GenWeight*SFlepton*SFbtag*SFPU*Luminosity*GenWeight*ttbbReweight*LumiFactor*sfTopPt);
                }
            }
        }//end of event loop
        ...
        if ( dataSetName.find(mainTTbarSample)!= string::npos && dataSetName.find("JES")==string::npos &&dataSetName.find("JER")==string::npos){
            ...
            // histo1D["Genweight_all"]->Write("Genweight_all");                  
...
void SplitDatasetPlotter(int nBins, float lScale, float plotLow, float plotHigh, string leptoAbbr, TFile *shapefile, TFile *errorfile, string channel, string sVarofinterest, string sSplitVar, float fbSplit, float ftSplit, float fwSplit, string xmlNom, string CraneenPath, string shapefileName)
{
    ...
    int nEntries;
    float ScaleFactor=1, NormFactor=1, Luminosity, varofInterest, splitVar, GenWeight=1, weight1=1, weight2=1, weight3=1, weight4=1, weight5=1, weight6=1, weight7=1, weight8=1, ttbar_flav=1, SFlepton=1, SFbtag=1, SFbtagUp=1, SFbtagDown=1, SFPU=1, SFPU_up=1, SFPU_down=1, sfTopPt=1;
    float PtLepton;
    bool reweight_ttbar = false;
    for (int d = 0; d < datasets.size(); d++)  //Loop through datasets
    {
        ...
        for(int s = fbSplit; s <= ftSplit; s+=fwSplit)
        {
            ...
            if(dataSetName.find("TTJets")!=string::npos){
                ...
                histo1D[("btag_Up"+numStr).c_str()] = new TH1F(("btag_Up"+numStr).c_str(),"btag_Up", nBins, plotLow, plotHigh);
                histo1D[("btag_Down"+numStr).c_str()] = new TH1F(("btag_Down"+numStr).c_str(),"btag_Down", nBins, plotLow, plotHigh);   
        ...
        cout<<"LumiFactor: "<<LumiFactor<<endl;
        ...
        for (int j = 0; j<nEntries; j++)
        {
            ...
            if(splitVar >= ftSplit) //Check if this entry belongs in the last bin.  Done here to optimize number of checks
            {
                ...
                histo1D[histoName]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*GenWeight);

                if(dataSetName.find("Data")!=string::npos || dataSetName.find("data")!=string::npos || dataSetName.find("DATA")!=string::npos || dataSetName.find("NP_overlay_Data")!=string::npos)
                {
                    MSPlot[plotname]->Fill(varofInterest, datasets[d], true,  LumiFactor*sfTopPt);
                    histo1D[histoName.c_str()]->Fill(varofInterest,eqlumi*LumiFactor*sfTopPt);
                }
                //outsiderange = false;
                else if (dataSetName.find(mainTTbarSample)!=string::npos&& dataSetName.find("JES")==string::npos && dataSetName.find("JER")==string::npos){
                    histo1D[("Genweight_tt"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*GenWeight);
                    histo1D[("weight1_tt"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight1);
                    histo1D[("weight2_tt"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight2);
                    histo1D[("weight3_tt"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight3);
                    histo1D[("weight4_tt"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight4);
                    histo1D[("weight5_tt"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight5);
                    histo1D[("weight6_tt"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight6);
                    histo1D[("weight7_tt"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight7);
                    histo1D[("weight8_tt"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight8);         
                    histo1D[("PU_Up"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU_up*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*GenWeight);
                    histo1D[("PU_Down"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU_down*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*GenWeight);
                    histo1D[("btag_Up"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtagUp*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*GenWeight);
                    histo1D[("btag_Down"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtagDown*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*GenWeight);
                    histo1D[("heavyFlav_Up"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight_up*LumiFactor*sfTopPt*GenWeight);
                    histo1D[("heavyFlav_Down"+stSplit).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight_down*LumiFactor*sfTopPt*GenWeight);    
                    MSPlot[plotname]->Fill(varofInterest, datasets[d], true, NormFactor*SFlepton*SFbtag*SFPU*Luminosity*ttbbReweight*LumiFactor*sfTopPt*GenWeight);
                }
                else if(dataSetName.find("MLM")==string::npos && dataSetName.find("Scale")==string::npos && dataSetName.find("JES")==string::npos && dataSetName.find("JER")==string::npos){ //ie. don't add the MLM dataset which is just used for matching
                    MSPlot[plotname]->Fill(varofInterest, datasets[d], true,NormFactor*SFlepton*SFbtagUp*SFPU*Luminosity*ttbbReweight*LumiFactor*sfTopPt*GenWeight);
                            // cout<<dataSetName<<" "<<varofInterest<<"  "<<NormFactor<<"  "<<SFlepton<<"  "<<SFbtag<<"  "<<SFPU<<"  "<<Luminosity<<"  "<<ttbbReweight<<"  "<<LumiFactor*sfTopPt<<"  "<<GenWeight<<endl;
                }
            }
                ...
                for(int s = fbSplit; s <= ftSplit; s+=fwSplit)
                {
                    if(splitVar>=s && splitVar<(s+fwSplit)) //splitVar falls inside one of the bins
                    {
                        ...
                        if(dataSetName.find("Data")!=string::npos || dataSetName.find("data")!=string::npos || dataSetName.find("DATA")!=string::npos || dataSetName.find("NP_overlay_Data")!=string::npos)
                        {
                            ...
                            histo1D[("Genweight_tt"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*GenWeight);
                            histo1D[("weight1_tt"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight1);
                            histo1D[("weight2_tt"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight2);
                            histo1D[("weight3_tt"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight3);
                            histo1D[("weight4_tt"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight4);
                            histo1D[("weight5_tt"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight5);
                            histo1D[("weight6_tt"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight6);
                            histo1D[("weight7_tt"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight7);
                            histo1D[("weight8_tt"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*weight8);         
                            histo1D[("PU_Up"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU_up*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*GenWeight);
                            histo1D[("PU_Down"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU_down*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*GenWeight);
                            histo1D[("btag_Up"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtagUp*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*GenWeight);
                            histo1D[("btag_Down"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtagDown*SFPU*Luminosity*eqlumi*ttbbReweight*LumiFactor*sfTopPt*GenWeight);
                            histo1D[("heavyFlav_Up"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight_up*LumiFactor*sfTopPt*GenWeight);
                            histo1D[("heavyFlav_Down"+numStr).c_str()]->Fill(varofInterest,NormFactor*SFlepton*SFbtag*SFPU*Luminosity*eqlumi*ttbbReweight_down*LumiFactor*sfTopPt*GenWeight);     
                            MSPlot[plotname]->Fill(varofInterest, datasets[d], true, NormFactor*SFlepton*SFbtag*SFPU*Luminosity*ttbbReweight*LumiFactor*sfTopPt*GenWeight);
                        }
                        else if(dataSetName.find("MLM")==string::npos && dataSetName.find("Scale")==string::npos && dataSetName.find("JES")==string::npos && dataSetName.find("JER")==string::npos){ //ie. don't add the MLM dataset which is just used for matching
                            MSPlot[plotname]->Fill(varofInterest, datasets[d], true, NormFactor*SFlepton*SFbtagUp*SFPU*Luminosity*ttbbReweight*LumiFactor*sfTopPt*GenWeight);
                            // cout<<dataSetName<<" "<<varofInterest<<"  "<<NormFactor<<"  "<<SFlepton<<"  "<<SFbtag<<"  "<<SFPU<<"  "<<Luminosity<<"  "<<ttbbReweight<<"  "<<LumiFactor<<"  "<<GenWeight<<endl;
        ...
        for(int s = fbSplit; s <= ftSplit; s+=fwSplit)
        {
            ...
            // }
        }//split categories loop
    }//end of dataset for loop

    GetScaleEnvelopeSplit(nBins, lScale, plotLow, plotHigh, leptoAbbr, shapefile, errorfile, channel, sVarofinterest,sSplitVar,fbSplit,  ftSplit ,fwSplit,xmlNom, CraneenPath, shapefileName, mainTTbarSample);
    GetMatchingSplit(nBins, lScale, plotLow, plotHigh, leptoAbbr, shapefile, errorfile, channel, sVarofinterest, xmlNom, CraneenPath, mainTTbarSample, otherTTbarsample,sSplitVar,fbSplit,  ftSplit ,fwSplit);
    errorfile->Close();
...
void DataCardProducer(TFile *shapefile, string shapefileName, string channel, string leptoAbbr, string xmlNom, float lScale){
    ...
    if(lScale>0){
        ...
        // datacardname = "datacard" + leptoAbbr + "_inc_L30_pt31.txt";          
...
void Split_DataCardProducer(TFile *shapefile, string shapefileName, string channel, string leptoAbbr, bool jetSplit, string sSplitVar1, float fbSplit1, float ftSplit1, float fwSplit1, string xmlNom, float lScale){
    ...
    string datacardname = "datacard" + leptoAbbr + "_" + "_JS" + ".txt";
    // string datacardname = "datacard" + leptoAbbr + "_" + "_JS" + "_L30_pt31.txt";
    card.open (datacardname.c_str());
