    ...
    string mainTTbarSample)
{
    ...
    for(int s = fbSplit; s <= ftSplit; s += fwSplit) {
        ...
        cout << "wrote weights in errorfile" << endl;
        shapefile->cd();
        string MEScalesysname = channel + numStr + sSplitVar + "__" + "ttbarTTX" + "__ttMEScale";
    ...
    string units)
{
    ...
    if(channel == "ttttmu")
        ...
        chanText = "Dilepton: ee+e#mu+#mu#mu";
    double ll_rw = 0.989;
    double bb_rw = 1.714;
            ...
            if((dataSetName.find("Data") != string::npos || dataSetName.find("data") != string::npos ||
                ...
                if(dataSetName.find(mainTTbarSample) != string::npos && dataSetName.find("JES") == string::npos &&
                    ...
                    histo1D["PU_Down"]->Fill(varofInterest, NormFactor * SFtrigger * SFlepton * SFbtag_light *
                            ...
                            eqlumi * ttbbReweight * LumiFactor);
                    histo1D["btag_lightUp"]->Fill(varofInterest, NormFactor * SFtrigger * SFlepton *
                            SFbtag_lightUpInflated * SFbtag_heavy * SFalphaTune * SFPU * SFTopPt * SFbehrends *
                    ...
                    histo1D["btag_lightDown"]->Fill(varofInterest, NormFactor * SFtrigger * SFlepton *
                            SFbtag_lightDownInflated * SFbtag_heavy * SFalphaTune * SFPU * SFTopPt * SFbehrends *
                            Luminosity * GenWeight * eqlumi * ttbbReweight * LumiFactor);
                    histo1D["btag_heavyUp"]->Fill(varofInterest, NormFactor * SFtrigger * SFlepton * SFbtag_light *
                            SFbtag_heavyUpInflated * SFalphaTune * SFPU * SFTopPt * SFbehrends * Luminosity *
                    ...
                    histo1D["btag_heavyDown"]->Fill(varofInterest, NormFactor * SFtrigger * SFlepton * SFbtag_light *
                            SFbtag_heavyDownInflated * SFalphaTune * SFPU * SFTopPt * SFbehrends * Luminosity *
                            GenWeight * eqlumi * ttbbReweight * LumiFactor);
...
//                                    Luminosity * ttbbReweight * LumiFactor);
                    ...
                    histo1D["btag_heavyDown_tttt"]->Fill(varofInterest, NormFactor * SFtrigger * SFlepton *
                            SFbtag_light * SFbtag_heavyDownInflated * SFalphaTune * SFPU * SFTopPt * SFbehrends *
                            Luminosity * GenWeight * eqlumi * ttbbReweight * LumiFactor);
    ...
    float lScale)
{
    ...
    for(int d = 0; d < datasets.size(); d++) // Loop through datasets
    {
        ...
        //        nTuple[dataSetName.c_str()]->SetBranchAddress("SFPU_up", &SFPU_up);
        //        nTuple[dataSetName.c_str()]->SetBranchAddress("SFPU_down", &SFPU_down);
         nTuple[dataSetName.c_str()]->SetBranchAddress("ttbar_flav", &ttbar_flav); //single lep sfTopPt
        ...
        for(int j = 0; j < nEntries; j++) {
            ...
            if(dataSetName.find("TTJets") != string::npos){
                if(ttbar_flav == 2) NormFactor *= 1.714;
                else NormFactor *= 0.989;
            }
            if(dataSetName.find("Data") != string::npos || dataSetName.find("data") != string::npos ||
                dataSetName.find("DATA") != string::npos) {
            ...
            for(int cutnum = 0; cutnum < nCuts; cutnum++) {
                if(dataSetName.find("Data") != string::npos || dataSetName.find("data") != string::npos ||
                    ...
                    if(cuts[cutnum] == 1) {
                        ...
                        if(split_ttbar && dataSetName.find("TTJets") != string::npos && ttbar_flav ==2) {
                            selecTable.Fill((datasets.size() + 1), (cutnum + 1), GenWeight * NormFactor * ScaleFactor);
                        }
                        else if(split_ttbar && dataSetName.find("TTJets") != string::npos && ttbar_flav !=2) {
                            selecTable.Fill((datasets.size()), (cutnum + 1), GenWeight * NormFactor * ScaleFactor);
                        }
                        else  {
    ...
    string units)
{
                ...
                ftSplit) // Check if this entry belongs in the last bin.  Done here to optimize number of checks
            {
                ...
                // outsiderange = false;
                    ...
                    histo1D[("btag_heavyUp" + stSplit).c_str()]->Fill(varofInterest, NormFactor * SFtrigger * SFlepton *
                            SFbtag_light * SFbtag_heavyUpInflated * SFalphaTune * SFPU * SFTopPt * SFbehrends *
                            Luminosity * eqlumi * ttbbReweight * LumiFactor * GenWeight);
                    ...
                    if(splitVar >= s && splitVar < (s + fwSplit)) // splitVar falls inside one of the bins
                    {
                        ...
                        if(dataSetName.find("Data") != string::npos || dataSetName.find("data") != string::npos ||
                            ...
                            histo1D[("btag_lightDown_tttt" + numStr).c_str()]->Fill(varofInterest, NormFactor *
                                    SFtrigger * SFlepton * SFbtag_lightDownInflated * SFbtag_heavy * SFalphaTune *
                                    SFPU * SFTopPt * SFbehrends * Luminosity * eqlumi * ttbbReweight * LumiFactor *
                                    GenWeight);
                            ...
                            histo1D[("btag_heavyDown_tttt" + numStr).c_str()]->Fill(varofInterest, NormFactor *
                                    SFtrigger * SFlepton * SFbtag_light * SFbtag_heavyDownInflated * SFalphaTune *
                                    SFPU * SFTopPt * SFbehrends * Luminosity * eqlumi * ttbbReweight * LumiFactor *
                                    GenWeight);
    ...
    GetScaleEnvelopeSplit(nBins, lScale, plotLow, plotHigh, leptoAbbr, shapefile, errorfile, channel,
    ...
    string units)
{
            ...
            for(int t2 = fbSplit2; t2 <= ftSplit2; t2 += fwSplit2) {
                ...
                if(dataSetName.find(mainTTbarSample) != string::npos && dataSetName.find("JES") == string::npos &&
                    ...
                    histo1D[("btag_lightUp" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str()] =
                        new TH1F(("btag_lightUp" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str(), "btag_lightUp",
                            nBins, plotLow, plotHigh);
                    histo1D[("btag_lightDown" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str()] =
                        new TH1F(("btag_lightDown" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str(),
                            "btag_lightDown", nBins, plotLow, plotHigh);
                    histo1D[("btag_heavyUp" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str()] =
                        new TH1F(("btag_heavyUp" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str(), "btag_heavyUp",
                            nBins, plotLow, plotHigh);
                    histo1D[("btag_heavyDown" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str()] =
                        new TH1F(("btag_heavyDown" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str(),
                            "btag_heavyDown", nBins, plotLow, plotHigh);
                    ...
                    histo1D[("btag_lightUp_tttt" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str()] =
                        new TH1F(("btag_lightUp_tttt" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str(),
                            "btag_lightUp_tttt", nBins, plotLow, plotHigh);
                    histo1D[("btag_lightDown_tttt" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str()] =
                        new TH1F(("btag_lightDown_tttt" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str(),
                            "btag_lightDown_tttt", nBins, plotLow, plotHigh);
                    histo1D[("btag_heavyUp_tttt" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str()] =
                        new TH1F(("btag_heavyUp_tttt" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str(),
                            "btag_heavyUp_tttt", nBins, plotLow, plotHigh);
                    histo1D[("btag_heavyDown_tttt" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str()] =
                        new TH1F(("btag_heavyDown_tttt" + numStr1 + sSplitVar1 + numStr2 + sSplitVar2).c_str(),
                            "btag_heavyDown_tttt", nBins, plotLow, plotHigh);
                }
            }
        }
        /////*****loop through entries and fill plots*****
        ...
        for(int j = 0; j < nEntries; j++) {
            ...
            if(dataSetName.find("Data") != string::npos || dataSetName.find("data") != string::npos ||
                ...
                histo1D[("btag_lightUp" + nameEnding).c_str()]->Fill(varofInterest, NormFactor * SFtrigger * SFlepton *
                        ...
                        eqlumi * ttbbReweight * LumiFactor * GenWeight);
                histo1D[("btag_lightDown" + nameEnding).c_str()]->Fill(varofInterest, NormFactor * SFtrigger *
                ...
                histo1D[("btag_heavyUp" + nameEnding).c_str()]->Fill(varofInterest, NormFactor * SFtrigger * SFlepton *
                        ...
                        eqlumi * ttbbReweight * LumiFactor * GenWeight);
                histo1D[("btag_heavyDown" + nameEnding).c_str()]->Fill(varofInterest, NormFactor * SFtrigger *
                ...
                histo1D[("PU_Down_tttt" + nameEnding).c_str()]->Fill(varofInterest, NormFactor * SFtrigger * SFlepton *
                        SFbtag_light * SFbtag_heavy * SFalphaTune * SFPU_down * SFTopPt * SFbehrends * Luminosity *
                        eqlumi * ttbbReweight * LumiFactor * GenWeight);
                histo1D[("btag_lightUp_tttt" + nameEnding).c_str()]->Fill(varofInterest, NormFactor * SFtrigger *
                        SFlepton * SFbtag_lightUp * SFbtag_heavy * SFalphaTune * SFPU * SFTopPt * SFbehrends *
                        Luminosity * eqlumi * ttbbReweight * LumiFactor * GenWeight);
                histo1D[("btag_lightDown_tttt" + nameEnding).c_str()]->Fill(varofInterest, NormFactor * SFtrigger *
                        SFlepton * SFbtag_lightDown * SFbtag_heavy * SFalphaTune * SFPU * SFTopPt * SFbehrends *
                        Luminosity * eqlumi * ttbbReweight * LumiFactor * GenWeight);
                histo1D[("btag_heavyUp_tttt" + nameEnding).c_str()]->Fill(varofInterest, NormFactor * SFtrigger *
                        SFlepton * SFbtag_light * SFbtag_heavyUp * SFalphaTune * SFPU * SFTopPt * SFbehrends *
                        Luminosity * eqlumi * ttbbReweight * LumiFactor * GenWeight);
                histo1D[("btag_heavyDown_tttt" + nameEnding).c_str()]->Fill(varofInterest, NormFactor * SFtrigger *
                        SFlepton * SFbtag_light * SFbtag_heavyDown * SFalphaTune * SFPU * SFTopPt * SFbehrends *
                        Luminosity * eqlumi * ttbbReweight * LumiFactor * GenWeight);
                histo1D[histoName]->Fill(varofInterest, NormFactor * SFtrigger * SFlepton * SFbtag_light *
                        SFbtag_heavy * SFalphaTune * SFPU * SFTopPt * SFbehrends * GenWeight * Luminosity * eqlumi *
    ...
    for(int s = fbSplit1; s <= ftSplit1; s += fwSplit1) {
        ...
        for(int t2 = fbSplit2; t2 <= ftSplit2; t2 += fwSplit2) {
            numStr2 = static_cast<ostringstream*>(&(ostringstream() << t2))->str();
            plotname = sVarofinterest + numStr1 + sSplitVar1 + numStr2 + sSplitVar2;
            histo1D[("ttbarTTX" + plotname).c_str()]->Write(
                (channel + numStr1 + sSplitVar1 + numStr2 + sSplitVar2 + "__ttbarTTX__nominal").c_str());
        }
    }
    ...
    float lScale)
{
    ...
    for(int d = 0; d < howmanyMC; d++) {
        ...
        if(dataSetName.find("Data") != string::npos || dataSetName.find("data") != string::npos ||
            ...
            dataSetName.find("Scale") != string::npos || dataSetName.find("MLM") != string::npos ||
            dataSetName.find("TTJets_aMCatNLO") != string::npos || dataSetName.find("TTW") != string::npos ||
            dataSetName.find("TTZ") != string::npos || dataSetName.find("TTH") != string::npos ||
            dataSetName.find("TTJets_SingleLept") != string::npos) {
            continue;
        } else if(dataSetName.find("tttt") != string::npos) {
            card << "tttt_norm              lnN          ";
        ...
        for(int t2 = fbSplit2; t2 <= ftSplit2; t2 += fwSplit2) {
            ...
            for(int j = 0; j < nDatasets; j++) {
                ...
                if(dataSetName.find("Data") != string::npos || dataSetName.find("data") != string::npos ||
                    ...
                    histoName = channel + numStr1 + sSplitVar1 + numStr2 + sSplitVar2 + "__" + "ttbarTTX" + "__nominal";
                    tempHisto = (TH1F*)shapefile->Get(histoName.c_str());
                    tempEntries = tempHisto->GetSumOfWeights();
                    card << static_cast<ostringstream*>(&(ostringstream() << tempEntries))->str() + " ";
                } else {
...
std::string plotVarNames(string s)
{
    ...
    if(s.find("nJets") != std::string::npos)
        ...
        rs = "p_{T}^{Jet2}";
    else if(s.find("BDT") != std::string::npos && s.find("trijet") == std::string::npos )
         rs = "BDT";
